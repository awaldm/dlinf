...
Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> maxpool2d_direct(
    ...
    int padding)
    {
        ...
        if (input.cols() != static_cast<Eigen::Index>(height) * static_cast<Eigen::Index>(width)) {
            throw std::runtime_error("MaxPool2D flattened input size mismatch");
        }
        ...
        if (kernel_size <= 0) {
            throw std::runtime_error("MaxPool2D kernel dimensions must be positive");
        }


        if (H_out <= 0 || W_out <= 0) {
            throw std::runtime_error("MaxPool2D output shape is empty");
        }
        ...
        for (Eigen::Index oc = 0; oc < input.rows(); ++oc) {
            for (Eigen::Index oy = 0; oy < H_out; ++oy) {
                for (Eigen::Index ox = 0; ox < W_out; ++ox) {
                    float max_val = -std::numeric_limits<float>::infinity(); // init output at that output tensor position. must be -inf
                    
                        // Now we are at the output position, iterate through the kernel for this pos and find the maximum
                        for (Eigen::Index ky = 0; ky < kernel_size; ++ky) { // y position inside kernel
                            for (Eigen::Index kx = 0; kx < kernel_size; ++kx) { // x position inside kernel
                                ...
                                if (iy >= 0 && iy < height && ix >= 0 && ix < width) { // get only valid positions
                                    ...
                                    max_val = std::max(input(oc, iy * width + ix), max_val); // update maximum for this kernel window position
                                }
                            }
                        }
                    
                    output(oc, oy * W_out + ox) = max_val; // we've iterated through the kernel, finalize the max value at the output coordinate
                }
            }
        }
